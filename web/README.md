...
LiveKit; the page has a language + voice picker and live pipeline lights. It's a
...
- **No on-screen transcript.** `RoomEvent.TranscriptionReceived` is used only to flash
  the pipeline nodes and confirm the agent joined — there is no caption UI.
...
- **Voice state**: an audio-energy analyser on the real mic + agent tracks drives a
  `listening / thinking / speaking` state — the center waveform reacts to whoever is
  talking and the pipeline nodes light accordingly. With no call, only idle visuals animate.
