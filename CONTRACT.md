...
## Prompts
Persona + full Fish domain knowledge live in `worker/prompts/system_prompt.md` (an
unnamed Fish Audio sales engineer — no persona name). The
private routing rubric is in `worker/prompts/runbook.md`. The side-call schema + rubric
are in `worker/prompts/extraction_prompt.md`. These are tuned with real call data.
